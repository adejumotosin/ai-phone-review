...
Original file is located at
    https://colab.research.google.com/drive/1dm1I9GgybWrDyERSZb1rlnrfbL-RWOSF
"""
...
# 4️⃣ Reviews Scraper
# -----------------------------
@st.cache_data(ttl=21600, show_spinner="💬 Fetching GSMArena reviews...") # 6h cache
def fetch_gsmarena_reviews(url, limit=20):
    ...
    try:
        ...
        review_blocks = soup.select(".user-thread .uopin")
...
# -----------------------------
@st.cache_data(ttl=43200, show_spinner="🤖 Summarizing with Gemini...") # 12h cache
def summarize_reviews(product_name, specs, reviews):
    ...
    Your job is to combine **GSMArena official specs** with **real user reviews**.

    Input:
    Phone: {product_name}

    Specs (from GSMArena, may be incomplete):
    {specs_context}
    ...
    ⚠️ IMPORTANT:
    - If a spec (Display, Processor, RAM, Storage, Camera, Battery, OS) is missing,
      infer it if possible or use "N/A".
    - Do NOT invent unrealistic specs.
    - Ensure all 7 fields are always present in the final JSON.
...
with col1:
    phone = st.text_input("Enter a phone name", "iPhone 15 Pro")
with col2:
