...
# UPDATED: Added a 'specifications_inferred' field to show the AI's search result.
...
# UPDATED: Prompt now enforces a two-step "Search and Review" process.
SYSTEM_PROMPT = f"""
...
with st.form("product_review_form"):
    ...
    # Model selection (Optional but useful for better results)
    model_choice = st.selectbox(
        "Select AI Model (Llama 3 70B is recommended for complex data/reasoning)",
    ...
    if not product_name:
        ...
        with st.spinner(f"Searching for specs and analyzing '{product_name}'..."):
            try:
                ...
                # Hide the product name from the JSON output and use the more structured one
                ...
                st.error("The AI failed to return a valid, structured JSON review. Try using a more specific product name.")
